...
| `animator.py` | 播放 ASCII 动画（悬浮贴纸/GIF/终端） | `python3.9 ${SKILL_DIR}/tools/animator.py --pet <slug> --action <action> --overlay` |
...
运行 `animator.py --pet <slug> --action greet --overlay` 播放欢迎动画（桌面贴纸形式）。
...
3. 根据用户指令映射到对应动作：
   ...
   - 玩耍 → `play`
   - 睡觉 → `sleep`
4. 运行 `animator.py --pet <slug> --action <action> --overlay` 播放动画
...
`animator.py` 支持多种播放模式，Agent 应优先使用 `--overlay`（桌面悬浮贴纸）：

| 模式 | 触发方式 | 适用场景 |
|------|---------|---------|
| **桌面贴纸** | `--overlay` | **推荐**。全局置顶悬浮窗，单窗口复用，动画播完自动回到肖像静止。需要 python3.9 |
| **GIF 动画** | `--gif <path>` | 生成动画 GIF 文件，在 IDE 中查看 |
| **静态输出** | 自动检测 / `--static` | 非交互式终端环境，输出肖像 + 逐帧展示 |
| **终端动画** | 在真实终端直接运行 | 用户在终端手动执行时自动使用 ANSI 动画 |

### Overlay 生命周期管理

**召唤宠物（首次启动，需 `block_until_ms: 0` 后台运行）：**
```bash
python3.9 ${SKILL_DIR}/tools/animator.py --pet <slug> --action greet --overlay
```

**切换动作（复用已有窗口，不会开新窗口）：**
```bash
python3.9 ${SKILL_DIR}/tools/animator.py --pet <slug> --action <action> --overlay
```

**关闭宠物（Agent 让宠物退出时）：**
```bash
python3.9 ${SKILL_DIR}/tools/animator.py --overlay-close
```

**自动退出：** 当 Agent 会话整体退出时，overlay 检测到父进程消亡会自动关闭。

悬浮窗在屏幕右下角显示，用户可拖拽至任意位置。动画循环 5 次后自动切换为宠物肖像静止状态。
